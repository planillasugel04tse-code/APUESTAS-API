...
def _remove_first_div(html: str, *, id_value: str | None = None, class_value: str | None = None) -> tuple[str, bool]:
    if id_value:
        pattern = rf'<div\b[^>]*\bid=["\']{re.escape(id_value)}["\'][^>]*>'
    else:
        pattern = rf'<div\b[^>]*\bclass=["\'][^"\']*\b{re.escape(class_value or "")}\b[^"\']*["\'][^>]*>'
    ...
    for tag in tag_re.finditer(html, pos):
        if tag.group(0).lower().startswith('</div'):
            depth -= 1
            if depth == 0:
                return html[:start] + html[tag.end():], True
        else:
            depth += 1
    return html, False
...
def _replace_first_div(html: str, *, class_value: str, replacement: str) -> tuple[str, bool]:
    ...
    for tag in tag_re.finditer(html, pos):
        if tag.group(0).lower().startswith('</div'):
            depth -= 1
            if depth == 0:
                return html[:start] + replacement + html[tag.end():], True
        else:
            depth += 1
    return html, False
...
def dashboard_fixed(today: date) -> HTMLResponse:
    ...
    if not replaced:
        html = html.replace('</section>', _PROVIDER_CARD + '</section>', 1)
