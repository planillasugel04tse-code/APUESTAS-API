...
CREATE TABLE IF NOT EXISTS telegram_messages (id INTEGER PRIMARY KEY AUTOINCREMENT, channel TEXT NOT NULL, message_id TEXT NOT NULL, raw_text TEXT NOT NULL, status TEXT NOT NULL, pick_id INTEGER, parsed_data TEXT, received_at TEXT NOT NULL, UNIQUE(channel, message_id));
CREATE TABLE IF NOT EXISTS telegram_signals (signal_id TEXT PRIMARY KEY, channel TEXT NOT NULL, message_id TEXT NOT NULL, tipster TEXT, raw_text TEXT NOT NULL, home_team TEXT NOT NULL, away_team TEXT NOT NULL, competition TEXT, market TEXT NOT NULL, selection TEXT NOT NULL, tipster_odds REAL, matched_event_id INTEGER, match_status TEXT NOT NULL, confidence REAL, stake REAL, analysis_result TEXT, created_at TEXT NOT NULL, betano_current_odds REAL, betano_odds_captured_at TEXT, line REAL, FOREIGN KEY(matched_event_id) REFERENCES matches(id), UNIQUE(channel, message_id));
CREATE TABLE IF NOT EXISTS team_match_history (
    ...
    UNIQUE(team, opponent, goals_for, goals_against, is_home, played_at, external_match_id)
);
CREATE INDEX IF NOT EXISTS idx_team_match_history_team_date ON team_match_history(team, played_at);
"""
