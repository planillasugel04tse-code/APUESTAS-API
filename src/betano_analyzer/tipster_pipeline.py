...
def enrich_tipster_pick(match_id: int, pick: TipsterPick, evidence: StatisticalEvidence, *, max_age_minutes: int = 180) -> EnrichedTipsterResult:
    """Attach fresh stored odds and analyze a tipster pick without external calls."""
    ...
    return EnrichedTipsterResult(analysis=analysis, market=MarketEnrichment(original=original, safer=safer))


def serialize_enriched(result: EnrichedTipsterResult) -> dict[str, Any]:
    analysis = result.analysis
    return {"analysis": {"status": analysis.status, "probability": analysis.probability, "fair_odds": analysis.fair_odds, "offered_odds": analysis.offered_odds, "edge": analysis.edge, "value_score": analysis.value_score, "safety_score": analysis.safety_score, "rejection_reasons": list(analysis.rejection_reasons), "pick": {"source": analysis.pick.source, "source_type": analysis.pick.source_type, "event": analysis.pick.event, "market": analysis.pick.market, "selection": analysis.pick.selection, "odds": analysis.pick.odds, "line": analysis.pick.line, "sport": analysis.pick.sport, "league": analysis.pick.league}, "safer": {"market": analysis.safer.safer_market, "selection": analysis.safer.safer_selection, "odds": analysis.safer.safer_odds, "edge": analysis.safer.safer_edge, "selected": analysis.safer.selected, "reason": analysis.safer.reason}}, "market": {"original": _quote_dict(result.market.original), "safer": _quote_dict(result.market.safer)}}


def _quote_dict(quote: MarketQuote | None) -> dict[str, Any] | None:
    if quote is None: return None
    return {"bookmaker": quote.bookmaker, "odds": quote.odds, "market": quote.market, "selection": quote.selection, "line": quote.line, "captured_at": quote.captured_at, "age_seconds": round(quote.age_seconds, 1)}
