...
def test_telegram_backtest_uses_core_evaluate_engine(monkeypatch):
    ...
    conn.executescript("""
        CREATE TABLE telegram_signals (signal_id TEXT PRIMARY KEY, channel TEXT, message_id TEXT, tipster TEXT,
            raw_text TEXT, home_team TEXT, away_team TEXT, competition TEXT, market TEXT, selection TEXT,
            tipster_odds REAL, matched_event_id INTEGER, match_status TEXT, confidence REAL, stake REAL,
            analysis_result TEXT, created_at TEXT);
        CREATE TABLE picks (id INTEGER PRIMARY KEY, match_id INTEGER, tipster_id INTEGER, original_market TEXT,
            original_selection TEXT, original_odds REAL, conservative_market TEXT, conservative_selection TEXT,
            conservative_odds REAL, confidence REAL, probability REAL, probability_source TEXT, created_at TEXT);
        CREATE TABLE pick_results (id INTEGER PRIMARY KEY, pick_id INTEGER UNIQUE, result TEXT, settled_at TEXT,
            actual_odds REAL, notes TEXT);
    """)
...
def test_arbitrage_uses_match_status_for_live_classification(monkeypatch):
    ...
    rows = [
        Row(match_id=1, bookmaker="Betano", market="1x2_ft", selection="home", line=None, odds=2.1, captured_at=fresh_ts,
            home_team="A", away_team="B", competition="Test League", kickoff="2026-09-10T20:00:00+00:00", status="live"),
        Row(match_id=1, bookmaker="Book2", market="1x2_ft", selection="draw", line=None, odds=4.5, captured_at=fresh_ts,
            home_team="A", away_team="B", competition="Test League", kickoff="2026-09-10T20:00:00+00:00", status="live"),
        Row(match_id=1, bookmaker="Book3", market="1x2_ft", selection="away", line=None, odds=4.5, captured_at=fresh_ts,
            home_team="A", away_team="B", competition="Test League", kickoff="2026-09-10T20:00:00+00:00", status="live"),
    ]
