...
from .surebet_scanner_features import scanner_css, scanner_js
...
def _remove_first_div(html: str, *, id_value: str | None = None, class_value: str | None = None) -> tuple[str, bool]:
    pattern = rf'<div\b[^>]*\b{id_value and "id" or "class"}=["\'][^"\']*{re.escape(id_value or class_value or "")}[^"\']*["\'][^>]*>'
    m = re.search(pattern, html, flags=re.I)
    if not m:return html,False
    start=m.start();pos=m.end();depth=1
    for tag in re.finditer(r'</?div\b[^>]*>',html[pos:],flags=re.I):
        if tag.group(0).lower().startswith('</div'):depth-=1
        else:depth+=1
        if depth==0:return html[:start]+html[pos+tag.end():],True
    return html,False

def _replace_first_div(html: str, *, class_value: str, replacement: str) -> tuple[str, bool]:
    pattern=rf'<div\b[^>]*\bclass=["\'][^"\']*\b{re.escape(class_value)}\b[^"\']*["\'][^>]*>'
    m=re.search(pattern,html,flags=re.I)
    if not m:return html,False
    start=m.start();pos=m.end();depth=1
    for tag in re.finditer(r'</?div\b[^>]*>',html[pos:],flags=re.I):
        ...
        if depth==0:return html[:start]+replacement+html[pos+tag.end():],True
    return html,False

def dashboard_fixed(today: date) -> HTMLResponse:
    if not active_account():return provider_accounts_page()
    html=dashboard_html(today)
    html,_=_remove_first_div(html,id_value='surebet-section')
    html=re.sub(r'<a\b[^>]*class=["\'][^"\']*\blive\b[^"\']*["\'][^>]*href=["\']#surebet-section["\'][^>]*>.*?</a>','<a class="live" href="/surebet">💰 SUREBET</a>',html,count=1,flags=re.I|re.S)
    html=re.sub(r'<a\b[^>]*class=["\'][^"\']*\bquick\b[^"\']*\bsurebet\b[^"\']*["\'][^>]*>.*?</a>','',html,count=1,flags=re.I|re.S)
    html,replaced=_replace_first_div(html,class_value='profile-card',replacement=_PROVIDER_CARD)
    if not replaced:html=html.replace('</section>',_PROVIDER_CARD+'</section>',1)
    html=html.replace('</script>',_PROVIDER_JS+'\nloadProviderStatus();\n</script>',1)
    html=html.replace('</style>',_CSS+'</style>',1)
    return HTMLResponse(html)

def surebet_page() -> HTMLResponse:
    base='''<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SUREBET · ANALISYS BETSTOTAL</title><style>:root{--bg:#f3f6fb;--ink:#172033;--line:#dfe5ee;--primary:#101827}*{box-sizing:border-box}body{font-family:Inter,Segoe UI,Arial,sans-serif;background:var(--bg);margin:0;color:var(--ink)}header{background:linear-gradient(135deg,#0b1220,#1c3157 65%,#2563eb);color:white;padding:26px 28px}main{max-width:1500px;margin:22px auto;padding:0 18px}.main-nav{display:flex;gap:9px;flex-wrap:wrap;margin-bottom:16px}.main-nav a{border:1px solid var(--line);border-radius:11px;padding:10px 14px;text-decoration:none;background:white;color:var(--ink);font-weight:800}.main-nav a.primary{background:var(--primary);color:white}.main-nav a.live{background:#fff1f2;color:#a51d2d}.muted{color:#697386}button{border:1px solid #dfe5ee;border-radius:9px;padding:9px 12px;font-weight:800;cursor:pointer}</style>'''
    enhanced_js=_JS+'\n'+scanner_js()
    enhanced_css=_CSS+'\n'+scanner_css()
    return HTMLResponse(base+f'<style>{enhanced_css}</style></head><body><header><h1>💰 SUREBET</h1><p>Centro de oportunidades · PRE-MATCH y LIVE · filtros avanzados</p></header><main>{_base_nav()}{_CARD}</main><script>{enhanced_js}</script></body></html>')
